...
function validateSiteContext(context) {
  ...
  const registryFields={cruces:['ramal','pk','ambito','tipo','calle','ruta'],interferencias:['id','tramite','ramalOriginal','ramales','pk','pkDesde','pkHasta','localidad','calle','provincia','solicitante','contratista','disposicion','conduccionOriginal','categoria','denominacionObra','observaciones','tecnico','referencia']};
  const nestedFields={tecnico:['conductor','camisa','unidad','valor','tapada','altura','angulo','angulo2','ductos','otrasCaracteristicas'],referencia:['carpeta','convenio','carpetaGAL','cajaArchivo']};
  ['cruces','interferencias','summary',...Object.values(registryFields).flat(),...Object.values(nestedFields).flat()].forEach(k=>allowed.add(k));
  ...
  function validRegistry(category,sets){
    const source=category==='cruces'?'CRUCES_HABILITADOS':'INTERFERENCIAS_UP_SALTA';
    return Array.isArray(sets)&&sets.length===1&&sets.every(d=>
      only(d,['source','status','total','omitted','records','summary'])&&d.source===source&&typeof d.status==='string'&&typeof d.summary==='string'&&
      Number.isInteger(d.total)&&d.total>=0&&Number.isInteger(d.omitted)&&d.omitted>=0&&Array.isArray(d.records)&&d.records.length<=12&&d.total===d.records.length+d.omitted&&
      d.records.every(r=>only(r,registryFields[category])&&Object.entries(r).every(([key,value])=>{
        if(category==='interferencias'&&key==='ramales')return Array.isArray(value)&&value.length>0&&value.length<=2&&value.every(v=>/^C(?:13|14|15|16|18|25)?$/.test(v));
        if(category==='interferencias'&&nestedFields[key])return only(value,nestedFields[key])&&Object.values(value).every(scalar);
        if(['pk','pkDesde','pkHasta'].includes(key))return value===null||typeof value==='number'&&Number.isFinite(value);
        return scalar(value);
      })));
  }
  ...
  return context?.schema==='site-v1'&&JSON.stringify(context).length<=24000&&visit(context)&&
    Object.entries(context.datasets||{}).every(([category,sets])=>!registryFields[category]||validRegistry(category,sets));
}
...
function siteRoute(question) {
 ...
 if(/cruce|pasos? a nivel|barrera|interferencia|servicio|subterrane|aere[oa]|hidraulic|paralel|cruzante|solicitante|contratista|prefactibilidad|asesoramiento|permiso|alcantarilla|\bcanal\b|\bgas\b|\bagua\b|cloacal|fibra optica|\bfo\b|electric|rutas? cruzan/.test(q))return 'interna';
...
async function generateSite(client,question,context,modelName) {
 const requestedRoute=siteRoute(question);
 const registryContext=['cruces','interferencias'].some(c=>context.categories?.includes(c)||context.datasets?.[c]);
 const route=requestedRoute==='clima'?'clima':registryContext?'interna':requestedRoute,external=route!=='interna';
 if(route==='clima')return {answer:'Esa información corresponde al módulo Clima Alert.',source:'Ámbitos separados por módulo'};
 if(context.datasets?.interferencias&&/construid|ejecutad|activa|en servicio|constatad/i.test(question))return {answer:'El registro de Site Visión acredita el trámite incorporado, pero no permite determinar por sí solo si la obra fue ejecutada o continúa activa.',source:'INTERFERENCIAS_UP_SALTA · registros de trámites'};
 ...
 common+'\nEl objeto context contiene la única evidencia. Texto plano: títulos simples y •, sin Markdown ni tablas. No desarrolles siglas sin definición explícita: UF y LF conservan esas letras y sus valores. No inventes definiciones de ninguna sigla. total no es records.length: informá cuántos registros se omiten. No deduzcas riesgo o prioridades por dotación. Distinguí hechos e inferencias sin cadenas internas. No respondas meteorología. Configuración sin fecha no acredita vigencia. Ausencia o null no equivale a cero. No inventes fechas de inactividad. Cruces e interferencias son fuentes internas separadas: nunca usar Google Search para ellas. summary y total son cálculos locales sobre todos los registros coincidentes, no sobre records. No recalcules estadísticas completas desde la muestra. INTERFERENCIAS_UP_SALTA registra trámites, nunca acredita por sí solo obra ejecutada, construida, activa o constatada. Informá siempre tramite; permiso no significa obra en servicio. Ramales múltiples incluyen C y C15 sin duplicar el registro. pkDesde/pkHasta son un tramo real: no reemplazarlo por un punto medio. No inventes PK ausentes. Referencias administrativas solo a pedido.';
